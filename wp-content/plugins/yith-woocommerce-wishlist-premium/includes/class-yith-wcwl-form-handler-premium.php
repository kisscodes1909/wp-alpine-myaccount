	...
	class YITH_WCWL_Form_Handler_Premium {
				...
				foreach ( $wishlists as $wishlist ) {
					if ( $wishlist->has_items() ) {
						foreach ( $wishlist->get_items() as $item ) {
							...
							/**
							 * APPLY_FILTERS: yith_wcwl_skip_wishlist_item_add_to_cart
							 *
							 * Skip specific product from being added in bulk to the cart
							 *
							 * @param bool                    Skip product
							 * @param WC_Product              $product  Product object
							 ...
							 * @return bool
							 */
							if ( apply_filters( 'yith_wcwl_skip_wishlist_item_add_to_cart', false, $product, $item, $wishlist ) ) {
								continue;
							}
		...
		public static function move_to_another_wishlist() {
			...
			if ( $wishlist && $destination_wishlist && $item ) {
				...
				// translators: 1. Destination wishlist name.
				wc_add_notice( apply_filters( 'yith_wcwl_moved_element_message', sprintf( __( 'Element correctly moved to %s', 'yith-woocommerce-wishlist' ), $destination_wishlist->get_formatted_name() ), 'success' ) );
			} else {
				wc_add_notice( apply_filters( 'yith_wcwl_move_product_to_wishlist_error_message', __( 'An error occurred while moving item to destination wishlist; please, try again', 'yith-woocommerce-wishlist' ) ), 'error' );
			}
