			...
			<?php if ( ! $instance['ajax_loading'] && ! empty( $products ) ) : ?>
				<ul class="cart_list product_list_widget">
					...
					foreach ( $products as $product_id => $info ) :
						...
						?>
						<li>
							<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'remove_from_wishlist', $product_id ), 'remove_from_wishlist' ) ); ?>" class="remove_from_all_wishlists" data-product-id="<?php echo esc_attr( $product_id ); ?>" data-wishlist-id="<?php echo 'yes' === $instance['show_default_only'] ? esc_attr( $default_wishlist ? $default_wishlist->get_id() : '' ) : ''; ?>">&times;</a>
