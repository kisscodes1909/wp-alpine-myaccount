...
class Frontend {
	...
	public function load_scripts() {
		...
		$instant_search_script = include_once CODEMANAS_TYPESENSE_ROOT_DIR_PATH . '/build/frontend/instant-search.asset.php';
		$autocomplete_script   = include_once CODEMANAS_TYPESENSE_ROOT_DIR_PATH . '/build/frontend/autocomplete.asset.php';
		$popup_script          = include_once CODEMANAS_TYPESENSE_ROOT_DIR_PATH . '/build/frontend/popup.asset.php';
		wp_register_script( 'cm-typesense-instant-search',
			$build_path . 'frontend/instant-search.js',
			array_merge( $instant_search_script['dependencies'], [ 'wp-util' ] ),
			$instant_search_script['version'],
			true );
		...
		wp_register_script( 'cm-typesense-autocomplete',
			$build_path . 'frontend/autocomplete.js',
			array_merge( $autocomplete_script['dependencies'], [ 'wp-util' ] ),
			$autocomplete_script['version'],
			true );
		...
		wp_register_script( 'cm-typesense-popup',
			$build_path . 'frontend/popup.js',
			array_merge( $popup_script['dependencies'], [ 'wp-util' ] ),
			$popup_script['version'],
			true );
		...
		wp_register_style( 'algolia-satellite', CODEMANAS_TYPESENSE_ROOT_URI_PATH . 'assets/vendor/css/algolia-satellite.min.css',
			false, $instant_search_script['version'], false );
		wp_register_style( 'cm-typesense-frontend-style',
			CODEMANAS_TYPESENSE_ROOT_URI_PATH . 'build/frontend/style-instant-search.css',
			[ 'algolia-satellite' ], $instant_search_script['version'],
			false );
	...
	public function load_popup() {
        ...
        <div class="cmswt-InstantSearchPopup">
            <div class="cmswt-InstantSearchPopup--results">
				...
				$shortcode_params = ! empty( $options['search_placeholder'] ) ? 'placeholder="' . $options['search_placeholder'] . '" ' : '';
				$shortcode_params .= ! empty( $options['no_of_cols'] ) ? 'columns="' . $options['no_of_cols'] . '" ' : '';
				$shortcode_params .= ! empty( $post_types ) ? 'post_types="' . $post_types . '" ' : '';
				$shortcode_params .= ! empty( $options['filter'] ) ? 'filter="' . $options['filter'] . '" ' : '';
				$shortcode_params .= ! empty( $options['results_per_page'] ) ? 'per_page="' . $options['results_per_page'] . '" ' : '';
				$shortcode_params .= ! empty( $options['sort_by'] ) ? 'sortby="' . $options['sort_by'] . '" ' : '';
				$shortcode_params .= ! empty( $options['pagination'] ) ? 'pagination="' . $options['pagination'] . '" ' : '';
				$shortcode_params .= ! empty( $options['sticky_first'] ) ? 'sticky_first="' . $options['sticky_first'] . '" ' : '';
