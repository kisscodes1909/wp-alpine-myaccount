			...
			foreach ( $fields as $field ) {
				switch ( $field['name'] ) {
					...
					case 'tags':
						$tags  = [];
						$tag_links = [];
						$terms = get_the_terms( $raw_data, 'post_tag' );
						if ( ! empty( $terms ) ) {
							foreach ( $terms as $term ) {
								$tag_links[] = get_term_link( $term );
								$tags[] = html_entity_decode( $term->name );
							}
						}
						$formatted_data['tags']     = is_array( $tags ) && ! empty( $tags ) ? array_values( array_unique( $tags ) ) : [];
						$formatted_data['tag_links'] = is_array( $tag_links ) && ! empty( $tag_links ) ? array_values( array_unique( $tag_links ) ) : [];
						break;
					default:
						break;
				}
