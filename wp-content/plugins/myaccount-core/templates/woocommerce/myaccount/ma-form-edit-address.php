...
<template id="edit-address" x-data>
    <div class="ma-modal">
        <div class="ma-modal__header">
            <div class="ma-modal__head">
                <h2 class="ma-modal__title ma-page-heading__title ma-page-heading__title--sm" x-text="$store.userAddress.form.title">Add Address</h2>
                <p class="ma-modal__description" x-text="$store.userAddress.form.action === 'edit' ? 'Update this saved address without losing your current delivery details.' : 'Add a new shipping or billing address to speed up future checkouts.'">Add a new shipping or billing address to speed up future checkouts.</p>
            </div>
            <button type="button" class="ma-modal__close ma-btn ma-btn--ghost" @click="$store.popup.closePopup()" aria-label="<?php esc_attr_e( 'Close', 'woocommerce' ); ?>">
