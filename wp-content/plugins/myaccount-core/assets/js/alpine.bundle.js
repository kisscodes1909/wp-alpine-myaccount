...
` + (formattedResult !== formattedValue ? `result of cast: ${formattedResult}` : ""));
  ...
  function viewOrderReturnsForm() {
    const data2 = getReturnsData();
    const eligibleItems = Array.isArray(data2.eligibleItems) ? data2.eligibleItems : [];
    const initialRequestType = Object.keys(data2.requestTypes || {})[0] || "return";
    return {
      eligibleItems,
      isSubmitting: false,
      errorMessage: "",
      ...
      async handleSubmit() {
        this.errorMessage = validateSelection(this.form, this.eligibleItems, data2.i18n || {});
        ...
        window.wp.ajax.post("submit_return_request", payload).done((response) => {
          ...
          this.isSubmitting = false;
          this.errorMessage = error2?.responseJSON?.data?.message || data2.i18n?.genericError || "Something went wrong. Please try again.";
          getStore("toast")?.addToast(this.errorMessage, "error");
        });
      }
    };
  ...
  function viewOrderReturns() {
    const data2 = getReturnsData();
    const initialRequests = Array.isArray(data2.requests) ? data2.requests : [];
    const initialEligibleItems = Array.isArray(data2.eligibleItems) ? data2.eligibleItems : [];
    return {
      requests: initialRequests,
      eligibleItems: initialEligibleItems,
      policy: data2.policy || {},
      canSubmit: Boolean(data2.policy?.is_eligible) && initialEligibleItems.length > 0,
      init() {
      ...
      openPopup() {
        ...
        const templateId = `ma-view-order-returns-popup-template-${data2.orderId}`;
      ...
      refreshEligibleItemsFromRequest(request) {
        ...
        data2.eligibleItems = this.eligibleItems;
