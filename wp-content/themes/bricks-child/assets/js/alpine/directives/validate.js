/**
 * Alpine Validation Directives
 * Custom directives for form validation UI
 * 
 * Available directives:
 * - x-validate-field: Add/remove error class based on validation state
 * - x-password-eye: Toggle password visibility icon
 * - x-validate-icon: Show tick/cross icons for validation feedback
 * - x-validate-message: Display validation error messages
 * - x-validate-error: Render reusable error icon + message block
 */

export function registerValidationDirectives() {
    
    // Directive: x-validate-field
    // Usage: <div x-validate-field="{message: errors.email, touched: touched.email}">
    Alpine.directive('validate-field', (el, { expression }, { evaluateLater, effect }) => {
        let getValidator = evaluateLater(expression);
        effect(() => {
            getValidator(errors => {
                const {message, touched} = errors;

                if (touched && message) {
                    el.classList.add('error');
                } else {
                    el.classList.remove('error');
                }
            })
        });
    });

    // Directive: x-password-eye
    // Usage: <div x-password-eye="showPassword" @click="showPassword=!showPassword">
    Alpine.directive('password-eye', (el, { expression }, { evaluateLater, effect }) => {
        let getValidator = evaluateLater(expression);

        effect(() => {
            getValidator(showPassword => {
                if (showPassword) {
                    el.innerHTML = `<svg class="cursor-pointer size-10" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                  <path stroke-linecap="round" stroke-linejoin="round" d="M3.98 8.223A10.477 10.477 0 0 0 1.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.863-.395M6.228 6.228A10.451 10.451 0 0 1 12 4.5c4.756 0 8.773 3.162 10.065 7.498a10.522 10.522 0 0 1-4.293 5.774M6.228 6.228 3 3m3.228 3.228 3.65 3.65m7.894 7.894L21 21m-3.228-3.228-3.65-3.65m0 0a3 3 0 1 0-4.243-4.243m4.242 4.242L9.88 9.88" />
                                </svg>`;
                } else {
                    el.innerHTML = `<svg class="cursor-pointer size-10"  xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                  <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                                  <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                                </svg>`;
                }
            })
        });
    });

    // Directive: x-validate-icon
    // Usage: <div x-validate-icon="{message: errors.email, touched: touched.email}">
    Alpine.directive('validate-icon', (el, { expression }, { evaluateLater, effect }) => {
        let getValidator = evaluateLater(expression);

        el.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="close w-10 h-10 text-red-600 bg-white">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" />
                            </svg>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="tick w-10 h-10">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" />
                            </svg>`;

        const closeEl = el.querySelector('.close');
        const tickEl = el.querySelector('.tick');

        effect(() => {
            getValidator(errors => {
                const {message, touched} = errors;

                if(message) {
                    closeEl.style.display = 'block';
                } else {
                    closeEl.style.display = 'none';
                }

                if(touched && !message) {
                    tickEl.style.display = 'block';
                } else {
                    tickEl.style.display = 'none';
                }
            })
        });
    });

    // Directive: x-validate-message
    // Usage: <span x-validate-message="{message: errors.email, touched: touched.email}">
    Alpine.directive('validate-message', (el, { expression }, { evaluateLater, effect }) => {
        let getValidator = evaluateLater(expression);

        effect(() => {
            getValidator(errors => {
                const {message, touched} = errors;

                if(message) {
                    el.innerHTML = message;
                } else {
                    el.innerHTML = '';
                }

                if(touched && message) {
                    el.style.display = 'block';
                } else {
                    el.style.display = 'none';
                }
            })
        });
    });

    // Directive: x-validate-error
    // Usage: <span x-validate-error="{message: errors.email, touched: touched.email}"></span>
    Alpine.directive('validate-error', (el, { expression }, { evaluateLater, effect }) => {
        let getValidator = evaluateLater(expression);

        el.classList.add('ma-form__error', 'items-center', 'gap-2');
        el.innerHTML = `<svg class="h-5 w-5 flex-none text-red-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true" focusable="false">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m0 3.75h.008v.008H12v-.008ZM21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                        </svg>
                        <span class="ma-form__error-text"></span>`;

        const textEl = el.querySelector('.ma-form__error-text');
        el.style.display = 'none';

        effect(() => {
            getValidator(errors => {
                const hasTouched = errors && Object.prototype.hasOwnProperty.call(errors, 'touched');
                const touched = hasTouched ? errors.touched : true;
                const message = errors && errors.message ? errors.message : '';
                const shouldShow = Boolean(message) && (touched === undefined || touched);

                if (textEl) {
                    textEl.textContent = message;
                }

                el.style.display = shouldShow ? 'inline-flex' : 'none';
            });
        });
    });
}
